// Given an abs-sorted array A and a number K, find all pairs of indics with sum
// = K, else (-1, 1)// Algo, should be Time-O(n) and space O(1).
...
// When opp sign, we start both from right. If (positive + negative) > K, then
// we reduce +ive component. If < K, we increase -ive component (by going left).
#include <iostream>
#include <vector>
...
void solveSameSign(const vector<int>& nos, vector<pair<int, int>>& ans, int K,
                   Comp comp) {
  int left = 0;
